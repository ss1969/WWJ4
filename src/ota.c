...
static void ota_main_rountine(const char *firmware_url) {
    char url[256] = {0};
    strncpy(url, firmware_url, sizeof(url));
    ...
    while (true) {
        ...
        switch (event.id) {
            case OTA_HTTP_GET_HEAD_DONE:
                done_len = 0;
                LOG("status %d total %u", luat_http_client_get_status_code(download.http), download.http->context_len);
                break;
            case OTA_HTTP_GET_DATA:
                ...
                LOG("ota data: %d, mem: all %d, now_free %d, min_free %d", done_len, all, now_free_block, min_free_block);
