...
static void IrqHandlerCoin(void *data, void *args) {
    ...
    if (SYSTICK() < MS2TICK(1000))
        return;

    if (svDbgCoin)
        LUAT_DEBUG_PRINT("COIN PIN %s @ %dms", ps ? "UP" : "DOWN", TICK2MS(SYSTICK()));
    ...
    We set detect limit to between 20~120ms  */
        ...
        if ((pps != coinSw1) && (SYSTICK() > MS2TICK(svCoinPulseWidthInLow) + tick) &&
            ...
            if (svDbgCoin)
                LUAT_DEBUG_PRINT("COIN++ %d, %dms", svCounterC, TICK2MS(SYSTICK() - tick));
        }
        else {
            if (svDbgCoin)
                LUAT_DEBUG_PRINT("COIN DROP %dms", TICK2MS(SYSTICK() - tick));
        }
    }
...
static void IrqHandlerPrizeExtCntMode1(void *data, void *args) {
    ...
    if (svDbgCoin)
        LUAT_DEBUG_PRINT("PIN_PRZ_EXT_COUNT %s @ %d", ps ? "UP" : "DOWN", TICK2MS(SYSTICK()));
    ...
    We set detect limit to between 20~120ms  */
        ...
        if ((pps != ticketerSw2) && (SYSTICK() > MS2TICK(svPrizePulseWidthInLow) + tick) &&
            ...
            if (svDbgCoin)
                LUAT_DEBUG_PRINT("PRIZE++ %d, %dms", svCounterD, TICK2MS(SYSTICK() - tick));
        }
        else {
            if (svDbgCoin)
                LUAT_DEBUG_PRINT("PRIZE DROP %dms", TICK2MS(SYSTICK() - tick));
        }
    }
...
static void gpio_dev_init(void) {
    ...
    gpio_cfg.irq_type = LUAT_GPIO_BOTH_IRQ;
    if (svDeviceType == 1)
        gpio_cfg.irq_cb = (void *)IrqHandlerPrizeExtCntMode1;
    if (svDeviceType == 2)
        gpio_cfg.irq_cb = (void *)IrqHandlerPrizeExtCntMode2;
    luat_gpio_open(&gpio_cfg);
