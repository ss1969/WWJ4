...
static const uint16_t st7789v_init_cmds[] = {
    ...
    0x02E0,0x03D0,0x0304,0x030D,0x0311,0x0313,0x032B,0x033F,0x0354,0x034C,0x0318,0x030D,0x030B,0x031F,0x0323,
    /* Negative Voltage Gamma Control */
    0x02E1,0x03D0,0x0304,0x030C,0x0311,0x0313,0x032C,0x033F,0x0344,0x0351,0x032F,0x031F,0x031F,0x0320,0x0323,
...
static int write_cmd_data_3w9bt1(luat_lcd_conf_t* conf, const uint8_t cmd, const uint8_t *data, uint8_t data_len)
{
    ...
    while(data_index != data_len){
        ...
        if(shift == 0) buf_index++;                             // shift == 0表示 data[data_index] 完全填满了上一个buffer位置了
...
int st7789v_set_direction(luat_lcd_conf_t* conf)
{
    ...
    if(conf->direction==0) direction_data = conf->opts->direction0;
    else if(conf->direction==1) direction_data = conf->opts->direction90;
    else if(conf->direction==2) direction_data = conf->opts->direction180;
    else direction_data = conf->opts->direction270;
...
luat_lcd_opts_t lcd_opts_st7789v = {
    .name = "st7789v",
